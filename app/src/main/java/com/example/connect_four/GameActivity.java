...
public class GameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pauseExitButton.setOnClickListener(v -> exitGame());
    ...
    private void onCellClick(int col) {
        ...
        for (int row = rows - 1; row >= 0; row--) {
            if (gridState[row][col] == 0) {
                ...
                // Update UI
                gridButtons[row][col].setBackgroundColor(isPlayer1Turn ? player1Color : player2Color);
    ...
    private boolean checkDirection(int row, int col, int rowStep, int colStep) {
        ...
        count += countPieces(row, col, rowStep, colStep);
        // Check in the opposite direction
        count += countPieces(row, col, -rowStep, -colStep);

        // Return true if count is 4 or more
        return count >= 4;
    }

    private int countPieces(int row, int col, int rowStep, int colStep) {
    ...
    private void showWinMessage(String winner) {
        ...
        // Add a delay before resetting the game (2 seconds)
